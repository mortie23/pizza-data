...
data generate_orders;
	length OrderID 8. OrderDate 8. PizzaID 8. IPAddress $16. PhoneNumber $10. monthnum 8.;
		...
		do PizzaID=1 to 2;
			...
			do while (ordervalid);
				...
				if probabilty < 0.1 then 
					ordervalid=0;
				OrderID=OrderID+1;
				IPAddress=put(uniform(1)*1000,3.)||'.'||put(uniform(1)*1000,3.)||'.'||put(uniform(1)*1000,3.)||'.'||put(uniform(1)*1000,3.);
				PhoneNumber='04'||put(uniform(1)*100000000,z8.);
				output;
			end;
		end;
	end;
...
data Orders;
	set generate_orders;
	keep OrderID OrderDate PizzaID IPAddress PhoneNumber;
run;
*	Testing a nice timeseries is generated;
proc summary data=generate_orders nway missing;
	class OrderMonth;
	output out=test_sum;
run;
